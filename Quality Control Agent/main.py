...
def load_prompts_template():
    """Load prompt files"""
    system_prompt = json.loads((PROMPTS_DIR / "Template" / "SYSTEM_Prompt.json").read_text(encoding="utf-8"))
    hidden_prompt = json.loads((PROMPTS_DIR / "Template" / "HIDDEN_Prompt.json").read_text(encoding="utf-8"))
...
def call_model_template(system_prompt, hidden_prompt, layout, client):
    """
    Call the LLM with system, hidden prompts plus optional images.
    ...
    contents.append({"type": "text", "text": f"Layout:"})
    contents.append({
        "type": "image_url",
        "image_url": {"url": f"data:image/png;base64,{layout}"}
    })
        
    usr_msg = {"role": "user", "content": contents}
...
def save_template(text_output, model_name):
    model_output_dir = OUTPUT_DIR / model_name / "Template"
    model_output_dir.mkdir(parents=True, exist_ok=True)
    (model_output_dir / "Template_Response.txt").write_text(text_output, encoding="utf-8")
...
if __name__ == "__main__":
    ...
    # layout_path = RAG_DIR / "layouts" / "layout2.png"
    
    # system_prompt, hidden_prompt = load_prompts_template()
    # layout = encode_image(layout_path)
    # client = initialize_client(config_path)
    # result = call_model_template(system_prompt, hidden_prompt, layout, client)
    # save_template(result, model_name)
